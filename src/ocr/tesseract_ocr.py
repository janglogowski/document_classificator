...
def ocr_image(path):
    ...
    data = pytesseract.image_to_data(proc, lang=OCR_LANG, output_type=pytesseract.Output.DICT)
    texts = [t for t in data["text"] if t.strip()]
    confs = [int(c) for c in data["conf"] if c != '-1']
    return " ".join(texts), confs

def ocr_image_from_array(arr):
    h, w = arr.shape[:2]
    if max(h, w) > MAX_DIM:
        scale = MAX_DIM / max(h, w)
...
def ocr_pdf(path):
    ...
    for pg in pages:
        ...
        text, confs = ocr_image_from_array(arr)
        all_text.append(text)
        all_confs.extend(confs)
    return "\n\n".join(all_text), all_confs

def ocr_file_with_timing(path, base_input):
    global _total_words
    start = time.time()
...
def batch_ocr(input_root, output_root):
    for dirpath, _, files in os.walk(input_root):
        ...
        for fn in files:
            if not fn.lower().endswith(IMG_EXTS + (PDF_EXT,)):
                continue
            src = os.path.join(dirpath, fn)
            dst = os.path.join(tgt_dir, os.path.splitext(fn)[0] + ".txt")
            if os.path.exists(dst):
                continue
            txt = ocr_file_with_timing(src, input_root)
